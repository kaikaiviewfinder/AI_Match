...
class OntologyFusion:
    ...
    def __init__(self, ontology_path: str | Path | None = None,
        ...
        # Load v2 parent fingerprints for clean scoring (P4: coarse-to-fine)
        self._parent_fingerprints = {}
        self._parent_centers = {}  # parent_id → (lat, lng)
        v2_path = Path(__file__).resolve().parent / "geo_ontology_v2.json"
        if "parent_id" in self.regions[0] and v2_path.exists():
            try:
                with open(v2_path, encoding="utf-8") as f:
                    v2 = json.load(f)
                self._parent_fingerprints = {r["id"]: r["fingerprint"] for r in v2["regions"]}
                self._parent_centers = {r["id"]: (r["center_lat"], r["center_lng"])
                                       for r in v2["regions"]}
            except Exception:
                pass
    ...
    def _sensor_filter(self, elevation_m, temperature_c, humidity_pct):
        """Relaxed sensor filter: only hard-filter on elevation.

        Temperature and humidity vary hugely by season — hard-filtering on them
        causes massive false negatives (e.g., Beijing July 25°C gets filtered
        because ontology range is [-5,15]). They are handled by soft affinity instead.
        """
        compatible = []
        for region in self.regions:
            e_min, e_max = region.get("elevation_range", [-9999, 99999])

            # Expand elevation range by 30% of width to reduce false negatives
    ...
    def _parent_aggregate(self, matches, sensor_elevation_m, sensor_temperature_c,
                         sensor_humidity_pct, ex, top_n_parents=12):
        """Two-stage coarse-to-fine: parent fingerprint ranking → sensor sub-region.

        Stage 1: Group sub-region matches by parent_id, use max score per parent.
                 This prevents score dilution from nearly-identical sibling fingerprints.
        Stage 2: Within each top parent, pick the child with best sensor affinity
                 (not best fingerprint — siblings are too similar for fingerprint alone).

        Returns re-ranked list of selected sub-region matches.
        """
        has_sensors = (sensor_elevation_m is not None or
                      sensor_temperature_c is not None or
                      sensor_humidity_pct is not None)

        # ── Stage 1: Parent-level aggregation ──────────────────────────
        parent_best = {}  # parent_id → best sub-region match
        for m in matches:
            rid = m.region_id
            region = self._region_by_id.get(rid, {})
            pid = region.get("parent_id", rid)  # fallback to self if no parent
            if pid not in parent_best or m.score > parent_best[pid].score:
                parent_best[pid] = m

        # Sort parents by best score
        parent_ranked = sorted(parent_best.values(), key=lambda m: m.score, reverse=True)
        n_parents = len(parent_ranked)

        if n_parents <= top_n_parents:
            # Few parents: use parent centers to avoid sub-region offset noise
            for m in matches:
                r = self._region_by_id.get(m.region_id, {})
                pid = r.get("parent_id", m.region_id)
                parent_center = self._parent_centers.get(pid)
                if parent_center:
                    m.center_lat, m.center_lng = parent_center
            ex.append(f"[PARENT-AGG] {n_parents} parent regions (≤{top_n_parents}), "
                     f"using parent centers")
            return matches

        # ── Stage 2: Sensor-based sub-region selection ──────────────────
        selected = []
        for parent_match in parent_ranked[:top_n_parents]:
            pid = parent_match.region_id
            region = self._region_by_id.get(pid, {})
            actual_pid = region.get("parent_id", pid)

            # Find all siblings of this parent
            siblings = []
            for m in matches:
                rid = m.region_id
                r = self._region_by_id.get(rid, {})
                r_pid = r.get("parent_id", rid)
                if r_pid == actual_pid:
                    siblings.append(m)

            if not has_sensors or len(siblings) <= 1:
                # No sensors or only child: use parent center, not sub-region offset
                parent_center = self._parent_centers.get(actual_pid)
                if parent_center:
                    parent_match.center_lat, parent_match.center_lng = parent_center
                selected.append(parent_match)
            else:
                # Compute sensor affinity for all siblings
                sib_affs = []
                for m in siblings:
                    r = self._region_by_id.get(m.region_id, {})
                    aff = self._sensor_affinity(
                        r, sensor_elevation_m, sensor_temperature_c, sensor_humidity_pct)
                    sib_affs.append((aff, m))
                sib_affs.sort(key=lambda x: -x[0])
                best_aff, best_sibling = sib_affs[0]
                worst_aff = sib_affs[-1][0]

                # If sensor can't discriminate (all siblings similar affinity),
                # fall back to parent center to avoid random offset
                if best_aff - worst_aff < 0.05:
                    parent_center = self._parent_centers.get(actual_pid)
                    if parent_center:
                        best_sibling.center_lat, best_sibling.center_lng = parent_center
                        ex.append(f"[PARENT-AGG] {best_sibling.region_name}: "
                                 f"sensor ambiguous (Δaff={best_aff-worst_aff:.3f}) → parent center")
                elif best_sibling.region_id != parent_match.region_id:
                    ex.append(f"[PARENT-AGG] {parent_match.region_name}: "
                             f"fp-best={parent_match.region_id}({parent_match.score:.3f}) → "
                             f"sensor-best={best_sibling.region_id}({best_sibling.score:.3f})")
                selected.append(best_sibling)

        # Also include strong-scoring sub-regions from lower-ranked parents
        # if they score higher than the weakest selected
        threshold = selected[-1].score if selected else 0
        for m in matches:
            if m not in selected and m.score > threshold:
                selected.append(m)

        selected.sort(key=lambda m: m.score, reverse=True)
        ex.append(f"[PARENT-AGG] {n_parents} parents → {len(selected)} sub-regions "
                 f"(top-{top_n_parents} parents + overflow)")
    ...
    def fuse(self,
        ...
        for region in candidates:
            # P4: use parent fingerprint for scoring (pure signal, no neighbor blend)
            score_region = region
            pid = region.get("parent_id")
            if pid and pid in self._parent_fingerprints:
                score_region = dict(region, fingerprint=self._parent_fingerprints[pid])
            match = self._score_region(score_region, elements)
        ...
        # ── Step 3.6: Parent-level aggregation (P4: coarse-to-fine) ────────
        # Prevent score dilution from near-identical sibling sub-regions.
        # Rank by parent, then pick best child via sensor affinity within each.
        matches = self._parent_aggregate(
            matches, sensor_elevation_m, sensor_temperature_c, sensor_humidity_pct, ex)
        best = matches[0]

  ...
  return (
    <div className="space-y-5 max-w-[1600px] mx-auto h-full flex flex-col">
      ...
      <div className="grid grid-cols-3 gap-4 text-sm">
        {[
          ['Matched (定位)', '#2563EB', s ? `±${(s.s_std_m).toFixed(1)}m` : '--'],
          ['GPS (断续)', '#F59E0B', `${gpsFixes.length} fixes`],
          ['Confidence', '#22C55E', s ? `${(s.confidence * 100).toFixed(0)}%` : '--'],
      ...
      {/* single full-width localization map (no VIO) */}
      <div className="flex-1 bg-white border border-[#E2E8F0] rounded-2xl overflow-hidden flex flex-col min-h-[500px]">
        <div className="flex items-center justify-between px-5 py-3 border-b border-[#E2E8F0]">
          <span className="text-sm font-semibold text-[#0F172A]">定位结果 · Fused Localization</span>
          <span className="text-xs text-[#94A3B8]">route + matched + GPS</span>
        </div>
        <div className="flex-1 relative min-h-[460px]">
          <TrajectoryMap variant="result" routePoints={routePts} pathHistory={pathHistory} gpsFixes={gpsFixes} gpsToLocal={gpsToLocal as any} />
        </div>
        <div className="px-5 py-2 border-t border-[#E2E8F0] flex gap-6 text-xs text-[#64748B]">
          <span className="flex items-center gap-1.5"><span className="w-2.5 h-0.5 bg-[#CBD5E1] inline-block" /> 路线</span>
          <span className="flex items-center gap-1.5"><span className="w-2.5 h-0.5 bg-[#2563EB] inline-block" /> 匹配定位</span>
          <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-full bg-[#F59E0B] inline-block" /> GPS(断续)</span>
        </div>
      </div>
      ...
      <div className="bg-white border border-[#E2E8F0] rounded-2xl p-4 flex items-center gap-6">
        ...
        <span className="text-xs text-[#64748B]">Route: {routePts.length > 1 ? (routePts[routePts.length-1]?.s || 0).toFixed(0) + 'm' : '--'}</span>
